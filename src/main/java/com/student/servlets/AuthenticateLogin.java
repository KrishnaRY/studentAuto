...
/**
 * This servlet checks whether the username,password and designation matches in
 * the database when it recieves a post request.If matched then it creates a
 * session and forwards to Welcome servlet otherwise redirects to
...
public class AuthenticateLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			String sql = "select * from t1 where Username='" + user
					+ "' and Password='" + Password + "' and Designation='"
					+ Designation + "'";
			...
			if (rs.next()) {
				...
				RequestDispatcher rd = request
						.getRequestDispatcher("/WEB-INF/ErrorPage.html");
